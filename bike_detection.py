...
bike_detection_model = joblib.load('models/bike_detection_model.pkl')
...
def find_bike_windows(img, classifier, y_start_stop=[0, 480], xy_window=(128, 128), xy_overlap=(0.85, 0.85) ):
    bike_windows = []
    ...
    for window in windows:
        ...
        if pred == 0:
            bike_windows.append(window)
    return bike_windows


def draw_bike_windows(img, windows):
    output = np.copy(img)
...
def draw_labeled_bboxes(img, labels):
    box=[]
    for bike_number in range(1, labels[1]+1):
        nonzero = (labels[0] == bike_number).nonzero()
...
def draw_labels_on_bike(img, boxes, threshold = 3):
...
def process_manager(img_pro,
    ...
    temp=[]
    for sc in scale:
        temp.append(find_bike_windows(img_pro, 
                                      bike_detection_model, 
    ...
    for box_set in temp:
        for box in box_set:
            windows.append(box)
    box_img = draw_bike_windows(img_pro, windows)
    img_bike, bike_box = draw_labels_on_bike(img_pro, 
                                             windows,
                                             threshold = threshold)
    return box_img, img_bike, bike_box

def bike_detetion_pro(img_pro, args):
    img = cv2.cvtColor(img_pro, cv2.COLOR_RGB2GRAY)
    img = cv2.resize(img, (128, 128))
    features =  np.squeeze(hog_features(img))
    pred = int(bike_detection_model.predict(features.reshape(1, -1)))
    if pred == 0:
        return 0
    elif pred == 1:
        return 1
